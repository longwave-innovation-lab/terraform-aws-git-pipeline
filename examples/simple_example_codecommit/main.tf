...
module "codecommit_path_filtered_pipeline" {
  depends_on = [aws_ssm_parameter.test_parameters]
  source     = "../.."

  source_file_path_filters         = ["path/to/match/*", "/app/*.js"]
  is_codecommit                    = true
  pipeline_custom_name             = "new_name_for_pipeline"
  repo_name                        = "your-repo-name"
  repo_branch                      = "your-branch-to-listen"
  add_manual_approval              = false
  force_delete_registry            = true
